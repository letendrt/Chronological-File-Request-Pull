...
# + accept/reject access requests 
# + revoke access to datasets 
...
dataset_doi = 'https://doi.org/MY.DATASET/API'                                  # Enter DOI link here in https or doi: format
api_token_origin = 'here_is_my_api_key'                                         # Fetch your API token and paste it here

'''
3 processing types:

1) To fetch requests on a dataset (and get the output CSV), set both
   'run_access_processor' and 'run_access_revoker' to False

2) To approve/reject requests (and create/update 'approved_tracker.csv'), set
   'run_access_processor' to True, and set 'run_access_revoker' to False

3) To revoke access to a dataset (and update the 'approved_tracker.csv'), set 
   'run_access_processor' to False, and set 'run_access_revoker' to True
'''

# Edit the 2 paramters below to reflect one of the 3 processing types described above
run_access_processor = False
run_access_revoker = False


# If running the access revoker, you must use the approved_tracker.csv file.
# Otherwise, fetch the link for the CSV file in which you granted/rejected 
# the request
chosen_csv_file = r'C:THIS/IS/MY/DIRECTORY/TO/FILE.csv'                           # Enter CSV file directory here - keep 'r' before string if on Windows
...
def approved_tracker(access_vetting):
    
    if os.path.isfile('approved_tracker.csv'):
        print('UPDATING APPROVED_TRACKER.CSV FILE')
        print()
        file = pd.read_csv('approved_tracker.csv')
        access_vetting['Access Granted Date-Time'] = str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))
        access_vetting['Revoking Access?'] = 'Pending'
        access_vetting['Access Revoked Date-Time'] = 'N/A'
        new_file = pd.concat([file, access_vetting], ignore_index = True)
        new_file.to_csv('approved_tracker.csv', index = False)
        print('FILE UPDATED')
    
    else:
        print('APPROVED_TRACKER.CSV DOES NOT YET EXIST')
        print('INITIATING FILE CREATION')
        print()
        access_vetting['Access Granted Date-Time'] = str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))
        access_vetting['Revoking Access?'] = 'Pending'
        access_vetting['Access Revoked Date-Time'] = 'N/A'
        access_vetting.to_csv('approved_tracker.csv', index = False)
        print(f'FILE CREATED IN {os.getcwd()}')



def access_revoker(chosen_csv_file, api_access, url_base_origin, headers_origin):
    
    frame = pd.read_csv(chosen_csv_file)
    access_revoking = frame.loc[frame['Revoking Access?'] == 'Revoke']
    revoking_list = formatter(access_revoking)
    
    for tasks in revoking_list:
        for k, v in tasks.items():
            url = f'{url_base_origin}/api/access/datafile/{v}/revokeAccess/{k}'       # Create URL for API call
            resp = requests.delete(url, headers = headers_origin)
            print(url)
            print(resp.json())
            print() 
    
    print('ACCESS REVOKED - UPDATING CSV SHEET')
    frame.loc[frame['Revoking Access?'] == 'Revoke', 'Revoking Access?'] = 'Revoked'
    frame.loc[frame['Revoking Access?'] == 'Revoked', 'Access Revoked Date-Time'] = str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))
    frame.to_csv('approved_tracker.csv', index = False)
    print('TRACKER SHEET UPDATED')
...
def access_processor(chosen_csv_file, api_access, url_base_origin, headers_origin):
    
    frame = pd.read_csv(chosen_csv_file)                                         # Load CSV file as a dataframe
    ...
    approved_tracker(access_vetting)
...
if run_access_processor == False and run_access_revoker == False:
    fetch_dataset_requests(dataset_doi, api_access, api_origin, url_base_origin, headers_origin)

if run_access_processor == True and run_access_revoker == False:
    access_processor(chosen_csv_file, api_access, url_base_origin, headers_origin)

if run_access_processor == False and run_access_revoker == True:
    access_revoker(chosen_csv_file, api_access, url_base_origin, headers_origin)
