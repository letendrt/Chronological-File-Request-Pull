...
# User Interface for bulk access request pull and data organisation 
...
def access_revoker(approved_tracker_file, api_access, url_base_origin, headers_origin):
    
    frame = pd.read_csv(approved_tracker_file)
...
text_val_65 = '3) Browse for the "approved_tracker.csv" sheet you wish to upload;'

text_val_7 = 'Currently: No chosen file!'
text_val_8 = '--Leave unchecked if creating request CSV sheet--'
text_val_9 = '--Must use an "approved_tracker.csv" sheet--'
...
label_65 = Label(root, text = text_val_65, fg = font_col, bg = uni_col, font = font_setting_2)
...
label_9 = Label(root, text = text_val_9, fg = font_col, bg = uni_col, font = font_setting_6)
...
label_8.place(relx = 0.085, rely = 0.596, anchor = 'w')
label_9.place(relx = 0.575, rely = 0.596, anchor = 'w')
...
def click_1():
    if choiceNum_1.get() == 1:
        ...
        task_revoker.config(state = NORMAL)
        ...
        print(choiceNum_1.get())
        task_revoker.config(state = DISABLED)
...
def click_3():
    if choiceNum_3.get() == 1:
        print(choiceNum_3.get())
        task_selection.config(state = NORMAL)
        file_button_1.config(state = DISABLED)
        file_button_1.place_forget()
        label_65.place_forget()
        label_7.place_forget()        
        
    
    else:
        print(choiceNum_3.get())
        task_selection.config(state = DISABLED)
        file_button_1.config(state = NORMAL)
        file_button_1.place(relx = 0.1, rely = 0.75, anchor = 'w', width = 300, height = 40)
        label_65.place(relx = 0.09, rely = 0.69, anchor = 'w')
        label_7.place(relx = 0.1, rely = 0.81, anchor = 'w')
...
def run_script():
    ...
    if choiceNum_1.get() == 1 and choiceNum_3.get() == 1:
        fetch_dataset_requests(dataset_doi, api_access, api_origin, url_base_origin, headers_origin)
        #print('Would create master sheet and pull requests for each restricted file in the dataset')
    
    if choiceNum_3.get() == 0 and choiceNum_1.get() == 1:
        approved_tracker_file = filename_1
        access_revoker(approved_tracker_file, api_access, url_base_origin, headers_origin)
        #print('Would revoke access')

    if choiceNum_1.get() == 0 and choiceNum_3.get() == 1:
        csv_granter_file = filename_1
        access_processor(csv_granter_file, api_access, url_base_origin, headers_origin)
        #print('Would approve/reject access requests')
...
task_revoker = tk.Checkbutton(
    root, 
    text = 'Check this box if Revoking Access', 
    command = click_3, 
    onvalue = 0, offvalue = 1, variable = choiceNum_3)
...
task_revoker.config(bg = uni_col, fg = font_col, font = font_setting_2)


# Deselect boxes by default
check_box_prod_dem.deselect()
task_selection.deselect()
task_revoker.deselect()


# Position Checkboxes in root window
check_box_prod_dem.pack(side = tk.BOTTOM, anchor = tk.E)
task_selection.place(relx = 0.045, rely = 0.56, anchor = 'w')
task_revoker.place(relx = 0.55, rely = 0.56, anchor = 'w')
